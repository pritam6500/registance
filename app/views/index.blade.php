...
<li ><a href="draft"><div class="full">Save Message</div></a></li>
...
<span class="add_cmnt" add="<?php echo $status['status_id']; ?>">Add Comment</span>
			<?php if(isset($feed['user_draft'])){
				for( $i=0; $i< sizeof($feed['user_draft']); $i++ ){
					if($status['status_id'] == $feed['user_draft'][$i]->msg_id ){ 
												$not_show=0;
													}
					}
					 }?>
			
		 <?php if($not_show==1){ ?> <span class="save_msg" id="<?php echo $status['status_id']; ?>">Save Message</span> <?php } ?>
